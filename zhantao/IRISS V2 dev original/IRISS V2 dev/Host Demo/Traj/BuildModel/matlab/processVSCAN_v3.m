function processVSCAN_v3(SCAN_NO_ACh)
% 2021-09-18 MJG Build Anatomical Model - version 3
%   Reads an ACh vscan into memory, segments it, models the PC, then plots
%   the full model and saves all params for the traj gen
%   ---- 
...
[endo_pts_px, corn_pts_px, lowestEndoPt, corneaThickness_px, corn_epi_px] = ...
    postprocess_cornea(segim == 2, 2.5);

% Process the iris; get iris and pupil data pts {O} [px] 
[iris_pts_px, pupil_pts_px] = postprocess_iris(segim == 3, lowestEndoPt, SDIR);
...
% Plot the complete eye model in {OCT} [mm]
plotOCTmodel(SDIR, SCAN_NO_ACh);
