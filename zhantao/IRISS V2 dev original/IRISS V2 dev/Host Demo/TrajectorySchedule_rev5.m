function [ X, Y, Pinv, IrisCenter, Zc_true, Za_true, Zp_true, Zi_true,...
           ...
           x_out, y_out, z_out, L4_out, S, vacf_out, za_eps, zp_eps ] = TrajectorySchedule_rev5...
...
% load our model data... 
% load([MDIR 'EyeModel_capsule_old.mat']);
% load([MDIR 'EyeModel_cornea_old.mat']);
load([MDIR 'EyeModel_cornea.mat']);
FX_IRISS_cornea = FX_IRISS; FY_IRISS_cornea = FY_IRISS; FZ_IRISS_cornea = FZ_IRISS; % Mia 09-09-21
load([MDIR 'EyeModel_capsule.mat']);
FX_IRISS_capsule = FX_IRISS; FY_IRISS_capsule = FY_IRISS; FZ_IRISS_capsule = FZ_IRISS; % Mia 09-09-21
...
% Obtain posterior surface fit % 2021-09-08 Mia 
OCTzRelDist = abs(OCTprobeZ_cornea - OCTprobeZ_post); 
pc_mm(:,3) = pc_mm(:,3); % - OCTzRelDist; % 2021-09-10 Mia 

% % Testing 2021-09-13 Mia 
% figure; 
% plot3(XYZ_mm_IRISSframe_capsule(:,1), XYZ_mm_IRISSframe_capsule(:,2), XYZ_mm_IRISSframe_capsule(:,3),'.','color', [51 102 204]/255,'MarkerSize',1); hold on; % 2021-09-08 Mia
% plot3(XYZ_mm_IRISSframe_cornea(:,1), XYZ_mm_IRISSframe_cornea(:,2), XYZ_mm_IRISSframe_cornea(:,3),'.','color', [51 102 204]/255,'MarkerSize',1); hold on; % 2021-09-08 Mia

tempPCpts = [pc_mm ones(size(pc_mm,1),1)] * TIO_post';
...
% Testing - Mia 2021-09-09
% [FX_grid_cornea, FY_grid_cornea] = meshgrid(FX_IRISS_cornea, FY_IRISS_cornea); 
% [FX_grid_capsule, FY_grid_capsule] = meshgrid(FX_IRISS_capsule, FY_IRISS_capsule); 
% FZ_cornea = griddata(FX_IRISS_cornea, FY_IRISS_cornea, FZ_IRISS_cornea, FX_grid_cornea, FY_grid_cornea); 
% FZ_capsule = griddata(FX_IRISS_capsule, FY_IRISS_capsule, FZ_IRISS_capsule, FX_grid_capsule, FY_grid_capsule); 
...
for k = 1:NoCycles
    ...
    %plot3(FX_IRISS_cornea, FY_IRISS_cornea, FZ_IRISS_cornea,'.','color', [51 102 204]/255,'MarkerSize',1); hold on; % 2021-09-09 Mia
    %plot3(FX_IRISS_capsule, FY_IRISS_capsule, (FZ_IRISS_capsule - OCTzRelDist),'.','color', [51 102 204]/255,'MarkerSize',1); hold on; % 2021-09-09 Mia
    plot(surfPC_IRISS); hold on; % Mia 2021-09-09
    plot(sfIRISS); hold on; % Mia 2021-09-09
    surf(X,Y,Za_work{k},'FaceColor','b','EdgeColor','none','FaceAlpha',.2);
